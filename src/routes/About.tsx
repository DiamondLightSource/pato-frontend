...
const AboutPage = () => (
  <VStack alignItems='start' className='about-text' mt='1em'>
    ...
    <section id='spa'>
      <Heading>SPA</Heading>
      <Text>
        Motion correction is performed with Relion's own implementation [{" "}
        <Link color='diamond.700' href='https://relion.readthedocs.io/'>
          https://relion.readthedocs.io/
        </Link>
        ]. CTF estimation uses CTFFind4 [{" "}
        <Link color='diamond.700' href='https://grigoriefflab.umassmed.edu/ctffind4/'>
          https://grigoriefflab.umassmed.edu/ctffind4/
        </Link>
        ]. Particle picking is performed with crYOLO using either the general model that is supplied
        with crYOLO, or a user-supplied model using the "Upload Models" button on PATo [{" "}
      ...
      <Text>
        For the first 50 000 particles 2D classification is run
        every 10 000 particles. The mask diameter is taken to be 10% larger than the particle
        diameter. After this first batch is complete all subsequent particles are batched into sets
        of 50 000 and 2D classification is performed on each set. On completion of the first batch
        the class ranker introduced with RELION 4 is used to score each class. This distribution is
        used to determine a score threshold which is used to select particles for 3D classification
        for all subsequent batched 2D classifications. Half of picked particles are retained in each
        batch.
      </Text>
      ...
      <Text>
        ...
        performed on batch sizes increasing by increments of 50 000 until a total of 200 000 particles
        is reached.
      </Text>

      <Text>
        The angular efficiency of the 3D classes is calculated using cryoEF [{" "}
        <Link color='diamond.700' href='https://www.mrc-lmb.cam.ac.uk/crusso/cryoEF/'>
          https://www.mrc-lmb.cam.ac.uk/crusso/cryoEF/
        </Link>
        ]. If the 3D classification produces a map with resolution better than 11 Å and cryoEF
        score better than 0.65, 3D refinement is run on the best class. This will be run at C1
        symmetry, and then a search will be run over different symmetries to determine the best fit.
      </Text>
    </section>
    <section id='tomogram'>
      ...
      <Text>
        Alignment and reconstruction are performed with AreTomo3 using the automated thickness
        determination and SART reconstruction implementation [{" "}
      ...
      <Text>
        Tomogram denoising is then performed with Topaz [{" "}
        <Link color='diamond.700' href='https://github.com/tbepler/topaz'>
          https://github.com/tbepler/topaz3
        </Link>
        ]. The denoised tomogram is used for the segmentation of membranes using membrain-seg [{" "}
        <Link color='diamond.700' href='https://github.com/teamtomo/membrain-seg'>
          https://github.com/teamtomo/membrain-seg
        </Link>
        ] and other features including ribosomes using easymode [{" "}
