...
const Breadcrumbs: FunctionComponent = (): JSX.Element => {
  ...
  const pathCrumbs = location.pathname.split("/").filter(name => { return name !== "" })
  if (pathCrumbs.length === 0) return (<></>);

  const currentPage = pathCrumbs.pop()
  let partialPath: Array<string> = Array(pathCrumbs.length + 1).fill("..")

  return (<Breadcrumb paddingBottom={4} id="breadcrumbs" separator=">">
    <BreadcrumbItem>
    ...
    <BreadcrumbItem >
      <BreadcrumbLink isCurrentPage key={currentPage}>{currentPage}</BreadcrumbLink>
    </BreadcrumbItem>
  </Breadcrumb >
  )
}
