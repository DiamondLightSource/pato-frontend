...
-   experimentTypeId now holds precedence over experimentType when
...
-   Motion correction is no longer displayed for processed tomograms,
    instead, they show up in the preprocessing step
...
-   Update session page text

## v1.29.0 (06/10/2025)

**Added**

-   Add MRC file uploads
...
-   Add atlas buttons to tomogram pages
...
-   Add option to download mmCIF files
...
-   Set particle count bin max to 300
...
-   User can toggle between displaying/hiding foil holes with no movies
    \@gmg29649
...
-   When a foil hole is selected, micrographs are displayed, instead of
    a list of movies

**Fixed**

-   Sorting is correctly applied between pages when a tomogram in the
    list of tomograms is clicked
-   Searching for a term that returns no results doesn\'t clear out the
    [search]{.title-ref} portion of your search parameters
...
-   Use new version of component library, use correct width/height for
    APNGs
...
-   Include particle count per resolution/defocus values graph in
...
-   Box size/mask diameter are both sent to the backend if form is
    prepopulated and unchanged
...
-   Cryolo, 2D classification, 3D classification enabled in reprocessing
    by default

**Added**

-   Relion reprocessing form now displays autocalculated values for box
    size/mask diameter
...
-   SPA reprocessing form now performs checks for maximum and minimum
    diameter when not stopping after CTF estimation
...
-   Pixel size on image is now displayed in angstroms instead of
    nanometres
...
-   Image directory is displayed in data collections rather than file
    templates
...
-   Unused fields `detector mode`, `c2 lens`, `first frame`,
    `last frame`, `refined magnification`, `number of particles`
...
-   Unused fields `numberOfImages`, `frameLength`, `frameDose`
...
-   Fix stale images when tomograms have repeating autoprocessing
    program IDs
...
-   Row components do not \"spill\" over boundaries of parent box when
    reducing page width
...
-   Displays message rather than ommitting whole classification row if
...
**Removed** - Unused columns (resolution in 2D classification, refined
tilt angle in SPA)
...
-   User can now filter out unselected classes; selected classes are
    displayed in green, unselected in red
...
-   Fixes auth redirection on Chrome, no orphan search parameters
    included
-   Performance and memory usage improvements to Molstar
-   Fixes empty white horizontal bar below footer on staging
    environments
...
-   API endpoints can be configured to be generated based on the current
    origin
...
-   Fixes motion correction pagination in SPA when moving through
    records quickly
...
-   Ultrawide monitors now display two columns of data in data
    collection pages
...
-   Classification rows with no data no longer display skeleton for
    pagination
...
-   Data statistics page now contains total motion, estimated resolution
    and particle count
...
-   Session list now displays microscope ID alongside human-readable
    name
...
-   Users can now search through collections (tomogram) in a data
    collection group
...
-   Every request displays loading bar, instead of loading bar state
...
-   Data for hidden processing jobs doesn\'t get downloaded until they
    are visible
-   \"Running\", \"Queued\" and \"Submitted\" job statuses are now
    supported
-   2D classification component shows message when no data is available
-   \"Beamlines\" in session table changed to \"Microscopes\"
-   Microscopes are now displayed as a human readable value instead of
    their beamline ID
-   In order to allow interactivity with plots, only clicking image/plot
    titles opens up the modal, instead of the whole card
...
-   Dedicated single particle analysis page (new particle picking and 2d
    classification components)
-   User can now pick between displaying all data collections, or only
    ones with valid tomograms

**Changed**

-   Visit number is displayed instead of session ID in data collection
    page
...
-   Motion correction for tomograms starts near the smallest tilt
    alignment angle (roughly the middle)

## v0.0.1-rc3 (30/11/2022)

**Added**

-   Motion correction can now be viewed for tomograms that are still
    being processed

**Changed**

-   Pagination for data collections accompanies the URL parameter
    properly
...
-   Visual tweaks
-   Data collections are now \"flipped\" through, from a data collection
    group, instead of selected from a list
